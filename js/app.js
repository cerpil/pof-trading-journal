...
document.addEventListener('DOMContentLoaded', async () => {
    if (SUPABASE_URL.includes('seu-projeto')) {
        Swal.fire({
            ...
            text: 'Configure suas credenciais do Supabase no arquivo js/app.js antes de usar!',
            icon: 'warning',
...
function showSection(section) {
    ...
    document.querySelector(`[data-section="${section}"]`).classList.add('active', 'bg-pof-metal/20', 'text-pof-light');
    document.querySelector(`[data-section="${section}"]`).classList.remove('text-pof-darksilver');
...
async function saveTrade(e) {
    ...
    if (trade.stop_loss) {
        ...
        const risk = riskPerUnit * trade.quantity;
    ...
    if (error) showError('Erro ao salvar: ' + error.message);
        ...
        Swal.fire({
            title: 'Trade Salvo!',
            text: `P&L: ${formatCurrency(trade.pnl)}`,
            icon: 'success',
            toast: true,
            position: 'top-end',
            timer: 3000,
...
function calculateStats() {
    if (trades.length === 0) return {
        netPnl: 0, winRate: 0, profitFactor: 0, expectancy: 0,
        avgWinner: 0, avgLoser: 0, totalTrades: 0,
        grossProfit: 0, grossLoss: 0, wins: 0, losses: 0,
...
function calculatePofScore() {
    ...
    // Consistency Score (0-30) - Based on last 10 trades
    const recentTrades = trades.slice(0, 10);
    const recentWins = recentTrades.filter(t => t.pnl > 0).length;
    const consistency = recentTrades.length > 0 ? (recentWins / recentTrades.length) * 100 : 0;
    const consistencyScore = Math.min(30, consistency * 0.3);
    
    // Risk Score (0-30) - Based on R-Multiple
    const avgR = trades.reduce((sum, t) => sum + Math.abs(t.r_multiple || 0), 0) / trades.length;
    const riskScore = Math.min(30, avgR > 1.5 ? 30 : avgR > 0.8 ? 20 : 10);
    
    return { 
        total: Math.round(profitScore + consistencyScore + riskScore), 
...
function updateCharts() {
    ...
    charts.equity = new Chart(ctx1, {
        ...
        options: {
            ...
            scales: {
                y: { grid: { color: 'rgba(192, 192, 192, 0.05)' }, ticks: { color: '#8A8A8A' } },
                x: { grid: { display: false }, ticks: { color: '#8A8A8A', maxTicksLimit: 6 } }
            }
        }
    });
    ...
    charts.daily = new Chart(ctx2, {
        ...
        data: {
            ...
            datasets: [{
                label: 'P&L',
                data: dailyData.data,
                backgroundColor: dailyData.data.map(v => v >= 0 ? '#10b981' : '#ef4444'),
                borderRadius: 4
            }]
        },
        options: {
            ...
            scales: {
                y: { grid: { color: 'rgba(192, 192, 192, 0.05)' }, ticks: { color: '#8A8A8A' } },
                x: { grid: { display: false }, ticks: { color: '#8A8A8A', maxTicksLimit: 8 } }
            }
        }
    });
    ...
    charts.setup = new Chart(ctx3, {
        type: 'doughnut',
        data: {
            labels: setupData.labels,
            datasets: [{
                data: setupData.data,
                backgroundColor: ['#C0C0C0', '#8A8A8A', '#4A5568', '#10b981', '#f59e0b', '#ef4444', '#3b82f6'],
                borderWidth: 0
            }]
        },
        options: {
            responsive: true,
            maintainAspectRatio: false,
            plugins: { legend: { position: 'right', labels: { color: '#8A8A8A', boxWidth: 12, font: { size: 10 } } } }
        }
    });
...
function calculateDailyPnl() {
    ...
    return { 
        labels: last30Days.map(d => new Date(d).toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit' })), 
        data: last30Days.map(d => daily[d]) 
    };
}
...
function calculateSetupStats() {
    ...
    trades.forEach(t => {
        ...
        setupPnl[setup] = (setupPnl[setup] || 0) + t.pnl;
    });
    return { labels: Object.keys(setupPnl), data: Object.values(setupPnl) };
}
...
function calculateHourlyStats() {
    ...
    trades.forEach(t => {
        ...
        hourly[hour] = (hourly[hour] || 0) + t.pnl;
    ...
    return { 
        ...
        data: hours.map(h => hourly[h] / (counts[h] || 1)) 
    };
}
...
function updateRecentTrades() {
    ...
    tbody.innerHTML = trades.slice(0, 5).map(trade => `
        <tr class="hover:bg-pof-metal/10 transition">
            <td class="px-6 py-4">${new Date(trade.date).toLocaleDateString()}</td>
            <td class="px-6 py-4 font-semibold text-pof-light">${trade.asset}</td>
            <td class="px-6 py-4">
                <span class="px-2 py-1 rounded text-xs font-semibold ${trade.direction === 'LONG' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-rose-500/20 text-rose-400'}">
                    ${trade.direction}
                </span>
            </td>
            <td class="px-6 py-4 text-pof-darksilver">${trade.setup || '-'}</td>
            <td class="px-6 py-4">R$ ${trade.entry_price.toFixed(2)}</td>
            <td class="px-6 py-4">R$ ${trade.exit_price.toFixed(2)}</td>
            <td class="px-6 py-4 font-semibold ${trade.pnl >= 0 ? 'text-emerald-400' : 'text-rose-400'}">
                ${formatCurrency(trade.pnl)}
            </td>
            <td class="px-6 py-4 text-pof-darksilver">${trade.r_multiple ? trade.r_multiple.toFixed(2) + 'R' : '-'}</td>
        </tr>
    `).join('');
}
...
function updateJournal() {
    ...
    document.getElementById('filterSetup').innerHTML = '<option value="">Todos Setups</option>' + 
        setups.map(s => `<option value="${s}">${s}</option>`).join('');
    
    tbody.innerHTML = trades.map(trade => `
        <tr class="hover:bg-pof-metal/10 transition">
            <td class="px-6 py-4 whitespace-nowrap">${new Date(trade.date).toLocaleDateString()} ${trade.time}</td>
            <td class="px-6 py-4 font-semibold text-pof-light">${trade.asset}</td>
            <td class="px-6 py-4">
                <span class="px-2 py-1 rounded text-xs font-semibold ${trade.direction === 'LONG' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-rose-500/20 text-rose-400'}">
                    ${trade.direction}
            ...
            <td class="px-6 py-4 text-pof-darksilver">${trade.r_multiple ? trade.r_multiple.toFixed(2) + 'R' : '-'}</td>
            <td class="px-6 py-4">
                <button onclick="deleteTrade('${trade.id}')" class="text-rose-400 hover:text-rose-300 transition">
...
function updateAnalytics() {
    ...
    document.getElementById('metricAvgRR').textContent = stats.avgLoser > 0 ? 
        '1:' + (stats.avgWinner / stats.avgLoser).toFixed(1) : '1:0';
    
    // Additional metrics
    const weekdayData = calculateWeekdayStats();
    const ctx1 = document.getElementById('weekdayChart').getContext('2d');
    if (charts.weekday) charts.weekday.destroy();
    charts.weekday = new Chart(ctx1, {
        type: 'bar',
        data: {
            ...
            datasets: [{
                ...
                backgroundColor: Object.values(weekdayData).map(v => v >= 0 ? '#10b981' : '#ef4444'),
                borderRadius: 4
            }]
        },
        options: {
    ...
    charts.rmulti = new Chart(ctx2, {
        type: 'bar',
        data: {
            labels: ['<-1R', '-1R a 0', '0 a 1R', '1R a 2R', '2R a 3R', '>3R'],
            datasets: [{
                label: 'Trades',
                data: [
                    rMultiples.filter(r => r < -1).length,
                    rMultiples.filter(r => r >= -1 && r < 0).length,
                    rMultiples.filter(r => r >= 0 && r < 1).length,
                    rMultiples.filter(r => r >= 1 && r < 2).length,
                    rMultiples.filter(r => r >= 2 && r < 3).length,
                    rMultiples.filter(r => r >= 3).length
                ],
                backgroundColor: '#C0C0C0',
                borderRadius: 4
            }]
    ...
    document.getElementById('tagsAnalysis').innerHTML = Object.entries(tagCounts)
        .sort((a, b) => b[1] - a[1])
        .map(([tag, count]) => `
            <span class="px-3 py-1 bg-pof-metal/30 rounded-full text-xs border border-pof-silver/20">
                ${tag} <span class="text-pof-silver font-bold ml-1">${count}</span>
            </span>
        `).join('') || '<p class="text-pof-darksilver text-sm">Adicione tags para análise</p>';
}
...
function renderCalendar() {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        const className = pnl > 0 ? 'bg-emerald-500/20 border-emerald-500/50' : 
                         pnl < 0 ? 'bg-rose-500/20 border-rose-500/50' : 
                         'bg-pof-metal/20 border-pof-metal/30';
        html += `
            <div class="aspect-square rounded-lg border ${className} p-1 flex flex-col items-center justify-center cursor-pointer hover:opacity-80 transition">
                <span class="text-[10px] text-pof-darksilver mb-auto w-full text-left">${day}</span>
                ${pnl !== 0 ? `<span class="text-[9px] font-bold ${pnl > 0 ? 'text-emerald-400' : 'text-rose-400'}">${pnl > 0 ? '+' : ''}${Math.round(pnl)}</span>` : ''}
            </div>
        `;
    }
    ...
    if (values.length > 0) {
        document.getElementById('calBestDay').textContent = formatCurrency(Math.max(...values, 0));
        document.getElementById('calWorstDay').textContent = formatCurrency(Math.min(...values, 0));
        document.getElementById('calAvgDay').textContent = formatCurrency(values.reduce((a, b) => a + b, 0) / values.length);
        document.getElementById('calPositiveDays').textContent = values.filter(v => v > 0).length;
    }
}
...
function renderPlaybooks() {
    ...
    const processedPlaybooks = playbooks.map(pb => {
        const pbTrades = trades.filter(t => t.setup === pb.name);
        return {
            ...pb,
            total: pbTrades.length,
            winRate: pbTrades.length > 0 ? (pbTrades.filter(t => t.pnl > 0).length / pbTrades.length * 100) : 0,
            pnl: pbTrades.reduce((sum, t) => sum + t.pnl, 0)
        };
    });
    
    grid.innerHTML = processedPlaybooks.map(pb => `
        <div class="metal-card rounded-xl p-6 cursor-pointer group">
            <div class="flex justify-between items-start mb-4">
                <div class="min-w-0">
                    <h3 class="font-display font-bold text-lg text-pof-light mb-1 tracking-wide truncate">${pb.name}</h3>
                    <p class="text-sm text-pof-darksilver line-clamp-2">${pb.description || 'Sem descrição'}</p>
                </div>
                <span class="px-3 py-1 rounded-full text-sm font-bold flex-shrink-0 ${pb.pnl >= 0 ? 'bg-emerald-500/20 text-emerald-400' : 'bg-rose-500/20 text-rose-400'}">
                    ${pb.pnl >= 0 ? '+' : ''}${formatCurrency(pb.pnl)}
                </span>
            </div>
            <div class="grid grid-cols-3 gap-4 pt-4 border-t border-pof-metal/30">
                <div class="text-center">
                    <div class="text-xl font-bold text-pof-silver">${pb.total}</div>
                    <div class="text-xs text-pof-darksilver uppercase tracking-wider">Trades</div>
                </div>
                <div class="text-center">
                    <div class="text-xl font-bold ${pb.winRate >= 50 ? 'text-emerald-400' : 'text-rose-400'}">${pb.winRate.toFixed(1)}%</div>
                    <div class="text-xs text-pof-darksilver uppercase tracking-wider">Win Rate</div>
                </div>
                <div class="text-center">
                    <div class="text-xl font-bold ${pb.pnl >= 0 ? 'text-emerald-400' : 'text-rose-400'}">${formatCurrency(pb.pnl)}</div>
                    <div class="text-xs text-pof-darksilver uppercase tracking-wider">P&L</div>
                </div>
            </div>
        </div>
    `).join('');
...
async function openPlaybookModal() {
    const { value: formValues } = await Swal.fire({
        title: 'Novo Playbook',
        html:
            '<input id="swal-name" class="swal2-input" placeholder="Nome do Setup">' +
            '<input id="swal-desc" class="swal2-input" placeholder="Descrição">',
        focusConfirm: false,
        showCancelButton: true,
        confirmButtonColor: '#C0C0C0',
        confirmButtonText: 'Criar Playbook',
        preConfirm: () => {
            return [
    ...
    if (formValues && formValues[0]) {
        ...
        if (error) showError('Erro: ' + error.message);
            ...
            playbooks.push(data[0]);
            renderPlaybooks();
            Swal.fire('Sucesso!', 'Playbook criado com sucesso.', 'success');
        }
    }
}
...
function updateSetupOptions() {
    ...
    select.innerHTML = '<option value="">Selecione...</option>' +
        playbooks.map(pb => `<option value="${pb.name}">${pb.name}</option>`).join('');
...
function exportToCSV() {
    if (trades.length === 0) {
        Swal.fire('Sem dados', 'Não há trades para exportar.', 'info');
        return;
    }
    ...
    link.setAttribute("download", `trades_pof_${new Date().toISOString().slice(0,10)}.csv`);
...
function filterTrades() {
    ...
    rows.forEach(row => {
        ...
        const rowSetup = cells[3].textContent;
        const rowDirection = cells[2].textContent.trim();
        const rowPnl = cells[7].textContent;
        const isWin = rowPnl.includes('R$') && !rowPnl.includes('-');
        
        let show = true;
        if (asset && !rowAsset.includes(asset)) show = false;
        if (setup && rowSetup !== setup) show = false;
        if (direction && !rowDirection.includes(direction)) show = false;
        if (result === 'win' && !isWin) show = false;
        if (result === 'loss' && isWin) show = false;
